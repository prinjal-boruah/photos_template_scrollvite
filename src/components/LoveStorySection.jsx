...
/* ─── Initial stacked state: deck with alternating peeks ───
   Each card sticks out ~5% alternating left/right so you
   can see the edges of all cards behind the top one.         */
...
/* ─── Scattered-timeline target positions (desktop) ─── 
   The cards zigzag diagonally from top-left to bottom-right,
   mimicking the reference image layout.                       */
...
/* Mobile positions (narrower spread) */
...
const Card = ({ photo, index, scrollYProgress, onClick, isMobile }) => {
  ...
  return (
    ...
    >
      {/* Polaroid card — 30% larger, rounded corners */}
      <div
      ...
      >
        ...
        <div className="pt-[6px] md:pt-[10px] pb-[2px] text-center">
          <p className="text-[9px] md:text-[11px] font-serif italic text-soft-brown/80 truncate">
            {photo.label}
          </p>
        </div>
      </div>

      {/* Date caption below the card */}
      <motion.p
...
/* ─── Dashed connector lines drawn as SVG ─── */
const ConnectorLines = ({ scrollYProgress, isMobile }) => {
...
const LoveStorySection = () => {
  ...
  /* Title fade-out as cards spread */
    ...
    <section ref={containerRef} className="relative w-full h-[300vh] bg-[#F4F1EA]">
      <div className="sticky top-0 w-full h-screen overflow-hidden flex flex-col items-center justify-center">
        ...
        >
          ...
          <p className="text-xs md:text-sm font-sans tracking-[0.3em] text-soft-brown mt-4 uppercase">
            Scroll to unfold
          </p>
        </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="w-full md:w-1/2 p-6 md:p-10 flex items-center justify-center bg-[#F4F0E8]">
                <div className="bg-white p-3 shadow-[0_4px_20px_rgba(0,0,0,0.08)] rotate-[-2deg] max-w-[320px] w-full">
                  ...
                  <div className="pt-3 pb-1 text-center">
                    <p className="text-xs font-serif italic text-soft-brown/60">
              ...
              <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                <h3 className="text-3xl md:text-4xl font-script text-dark-brown mb-2">
                  {photos[selectedPhotoIndex].label}
                </h3>
                <p className="text-xs font-sans uppercase tracking-[0.2em] text-gold mb-1">
                  📍 {photos[selectedPhotoIndex].location}
                </p>
                <p className="text-xs font-sans uppercase tracking-[0.15em] text-soft-brown/60 mb-8">
                  {photos[selectedPhotoIndex].caption}
                </p>
                ...
                <div className="flex items-center gap-4 mt-10 pt-6 border-t border-dark-brown/10">
                  <button
                    onClick={prevPhoto}
                    className="flex items-center gap-1 text-xs uppercase tracking-widest text-soft-brown hover:text-gold transition-colors font-sans"
                  >
                    <ChevronLeft size={16} strokeWidth={1.5} /> Prev
                  </button>
                  <span className="text-soft-brown/30 text-xs">
